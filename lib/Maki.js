...
var Maki = function( config ) {
  ...
  var components = glob.sync( self.app.set('views') + '/components/*.jade' ).map(function(file) {
    var name = path.basename( file , '.jade');
    return 'maki.components[\''+name+'\'] = ' + jade.compileFileClient( file , {
      require: false
    });
  }).join('\n');
  
  var browserify = require('browserify-middleware');
  var stringify  = require('browserify-string');
  
  self.app.get('/js/bundle.js', function(req, res, next) {
    res.setHeader('content-type', 'application/javascript');
    
    var bundle = stringify( components , {
      cache: true,
      precompile: true
