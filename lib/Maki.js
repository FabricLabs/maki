          ...
          html: function() {
            ...
            async.map( Object.keys(resource.requires) , function( requirement , done ) {
              ...
              res.render( resource.templates[ method ] , locals , function( err , body , next ) {
                ...
                console.log('[WARNING] no rendered body for ', err.view , data );
                console.log('[TODO] configure bootstrap to generate a template');
...
Maki.prototype.bootstrap = function( done ) {
  ...
  for (var name in self.resources) {
    ...
    for (var method in resource.templates) {
      ...
      if (!found) {
        // TODO: log [WARNING] when not in production mode
        console.log('[BOOTSTRAP]', resource.name , template + defaultEngine );
