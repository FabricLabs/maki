...
Maki.prototype.start = function( done ) {
  ...
  self.httpd.on('error', function(err) {
    console.log('error event: ', err);
  });
  
  self.httpd.on('listening', function() {
    var address = self.httpd.address();
    
    var port = address.port;

    // use of `0` allows for assignment of a random available port
    self.config.services.http.port = port;
  ...
  self.httpd.listen( self.config.services.http.port );
