...
var Maki = function( config ) {
  ...
  //self.app.use( require('connect-flash')() );
  self.app.use( require('flashify') );
  ...
  var provider = function(req, res, next) {
    ...
    console.log('v', v);
    console.log('verbs[ v ]', resource.verbs[ v ] );
    
    //resource.verbs['options'] = [ resource.verbs['get'][0] ];
    ...
    resource.as( facade ).perform( method ).with( query ).exec(function(err, instance) {
