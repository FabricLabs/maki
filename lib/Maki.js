  ...
  var provide = function() {
    return function( req, res, next ) {
      ...
      res.provide = function( name , data ) {
        ...
        console.log(resource.verbs);

        Object.keys( resource.verbs[ v ] ).forEach(function( m ) {
          var regex = pathToRegex( m );
          if (regex.test( req.path )) {
            console.log('matched ' , m , resource.methods[m] );
            
            method = resource.methods[ m ];
            return;
          }
        });
        
        console.log('evaled, method ' , method)
        ...
        res.format({
          ...
          html: function() {
            ...
            locals[ resource.collection ] = data;
...
Maki.prototype.start = function( done ) {
  ...
  Object.keys( self.resources ).forEach(function( r ) {
    ...
    Object.keys( resource.routes ).forEach(function( p ) {
      ...
      self.resources[ r ].verbs[ map[p] ][ regex ] = p;
