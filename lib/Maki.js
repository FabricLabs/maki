...
Maki.prototype.serve = async function (services) {
  let self = this;
  if (typeof services === 'string') services = [ services ];
  if (self.served) return this;
  ...
  let promises = services.map(function (name) {
    return (async function () {
      if (!self.suppress) console.log('[SERVICE]'.bold.green , '['+name+']'.bold , 'preparing...'.bold);

      let config = self.config.services[name] || {};
      let Service = self._services[name];
      let service = new Service(config);

      service.on('started', function() {
        let address = service._server.address();
  ...
  let running = await Promise.all(promises);
...
Maki.prototype.start = async function () {
    ...
    if (self.config.oplog) {
      ...
      if (self.debug) {
        self.oplog.count({}, function (err, count) {
          if (err) console.error('Failure to count oplog:', err);
  ...
  await self.serve(Object.keys(self._services));

  console.log('[MAKI]', 'ready!');
...
Maki.prototype.destroy = async function () {
  console.log('[MAKI]', 'stopping...');
  let self = this;
  ...
  await Promise.all([
    services.map(function (name) {
      return (async function () {
        return self.services[name].destroy();
      }());
    })
  ]);
