...
Resource.prototype.attach = function( maki ) {
  ...
  self.Schema = maki.datastore.define(self.plural, self.attributes);
  ...
  self.options.plugins.forEach(function(plugin) {
    console.log('NOT IMPLEMENTED: would have loaded plugin', plugin);
    // self.Schema.plugin( plugin );
  });

  //self.Model = maki.mongoose.model( self.name , self.Schema );
  self.Model = maki.datastore.model(self.plural, self.Schema);
...
Resource.prototype.query = function( q , opts , complete ) {
  ...
  function executeMethod( q , done ) {
    var query = Model.find( q );//.sort( opts.sort || { _id: -1 } );
    if (opts.populate) {
      console.log('error: populate not implemented');
