...
Resource.prototype.patch = function( query , params, complete ) {
  ...
  function executeMethod( doc , done ) {
    Model.findOne( query ).exec(function(err, doc) {
      ...
      console.log('patches:', params);
      console.log('before:', doc);
      jsonpatch.apply( doc , params );
      console.log('after:', doc);

      doc.save(function(err, mot) {
        console.log('saved, err:', err );
        console.log('saved, mot:', mot );
        if (err) return complete(err);
        return done( err , mot );
      });
    });
...
Resource.prototype.update = function( query , params , complete ) {
  ...
  function executeMethod( params , done ) {
    Model.findOne( query ).exec(function(err, doc) {
      ...
      var patches = jsonpatch.generate( observer ).filter(function(x) {
        if (x.op === 'replace') return false;
        return x;
      }).map(function(x) {
        if (x.op === 'add') x.op = 'replace';
        return x;
      });
