...
var Resource = function( name , options ) {
  ...
  Object.keys( self.options.attributes ).forEach(function( f ) {
    ...
    if (attribute.type && !~['File'].indexOf( attribute.type )) {
      console.log('YES', attribute.type );
...
Resource.prototype.attach = function( maki ) {
  ...
  console.log(self.name , 'options.attributes', self.options.attributes );
  
  Object.keys( self.attributes ).forEach(function( f ) {
    ...
    if ( field.type && field.type.name ) {
      console.log('sup, ' , field.type.name );
    }
