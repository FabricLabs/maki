...
var Resource = function( name , options ) {
  ...
  // whitelist of types
  for (let name in self.options.attributes) {
    let attr = self.options.attributes[name];
    switch (attr.type) {
      case 'Map':
        // TODO:": "Define the `Map` type and implement in Fabric Core.
        self.options.attributes[name].type = 'Mixed';
        break;
      case 'Number':
        self.options.attributes[name].type = Number;
        break;
      case 'String':
        self.options.attributes[name].type = String;
        break;
    }
  }
...
Resource.prototype.attach = function( maki ) {
  ...
  if (!self.internal) {
    if (maki.mongoose.models[self.name]) {
      // TODO: compare and reconcile?
      console.warn(`[RESOURCE:${self.name.toUpperCase()}]`, `Model already exists!`);
      self.Model = maki.mongoose.models[self.name];
    } else {
  ...
  if (self.source) {
    ...
    if (typeof( self.source ) === 'function') {
      ...
      if (!fs.existsSync(self.source)) {
        console.error('[MAKI:RESOURCE]', 'source does not exist:', self.source);
      }
      
      if (fs.statSync(self.source).isDirectory()) {
