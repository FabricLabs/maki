...
var Resource = function( name , options ) {
  ...
  self.store = options.store || false;
  ...
  if ((self.options.internal && !self.options.store) || self.options.static) return;
...
Resource.prototype.attach = function( maki ) {
  ...
  if ((self.internal && !self.store) || self.static) return;
...
Resource.prototype.create = function( doc , params , complete ) {
  ...
  Model.findOne( query ).exec(function(err, instance) {
    ...
    // execute any necessary middlewares before "Schema" creation is completed
