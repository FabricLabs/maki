...
var Resource = function( name , options ) {
  ...
  Object.keys( options.attributes ).forEach(function( f ) {
    var attribute = options.attributes[ f ];
...
Resource.prototype.attach = function( maki ) {
  ...
  if (self.source) {
    console.log('self.source yes')
    
    try {
      console.log('trying...');
      
      var file = fs.readFileSync( self.source );
      var data = JSON.parse( file );
      
      self.data = data;
      
      self.Model.find = function( q ) {
        console.log('fake find');
        return {
          exec: function( cb ) {
            cb( null , self.data );
          },
      ...
      console.log('self.Model.find', self.Model.find )
