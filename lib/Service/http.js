    ...
    Object.keys( resource.attributes ).forEach(function( f ) {
      ...
      if (~['File'].indexOf( attribute.type )) {
        resource.pre('create', function( done ) {
          ...
          var streams = [];
          resource._meta.files.forEach(function(fileField) {
            console.log('checking for meta filefield', fileField);
            if (self.files[ fileField ]) {
              console.log('self fileField exists!' , fileField);
              streams.push(function(err, complete) {
                self.attach( fileField , self.files[ fileField ] , complete );
              });
            }
          });
    ...
    Object.keys( resource.routes ).forEach(function( p ) {
      ...
      switch (p) {
        ...
        case 'create':
          ...
          console.log('setting up PUT for', resource.name , 'at', regex + '/:id' );
