...
HTTP.prototype.attach = function( maki ) {
  ...
  */
  ...
  function defineAttributeProperties( attribute ) {
    if (!attribute.hasOwnProperty('_rows')) {
    ...
    if (!attribute.hasOwnProperty('_ref')) {
    ...
    if (!attribute.hasOwnProperty('_type')) {
    ...
    if (!attribute.hasOwnProperty('_default')) {
        ...
        var executor = function(req, res, next) {
          ...
          if (filter) {
            try {
              ...
              console.error(`Error parsing filter (${JSON.stringify(filter)}):`, e);
            }
          }
          // END ALARM.  SCROLL UP.
...
HTTP.prototype.destroy = async function() {
  return this.maki.server.close();
};
