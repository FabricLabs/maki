...
HTTP.prototype.provide = function() {
  ...
  return function( req , res , next ) {
    ...
    res.provide = function( name , data , handlers ) {
      ...
      if (self.maki.debug) console.log('res.provide()', name , data , handlers );
      ...
      console.log('provide, handlers:', handlers);

      Object.keys( handlers ).forEach(function( f ) {
        console.trace('specific handler', req.m , req.path , req.body ,  handlers[ f ][ req.m ] );
        if (handlers[ f ][ req.m ]) {
...
HTTP.prototype.start = function( started ) {
  ...
  pem.createCertificate({
    ...
    self.maki.server = self.maki.httpd.listen( self.maki.config.services.http.port , function() {
      ...
      if (!self.maki.suppress) console.log('[SERVICE]'.bold.green , '[HTTP]'.bold , 'listening'.green, 'for', '[http]      '.bold, 'on ' , 'http://' + address.address + ':' + address.port  );

      self.address = address;

      self.maki.serverSPDY = self.maki.spdyd.listen( sslPort , function() {
        var address = self.maki.spdyd.address();
        if (!self.maki.suppress) console.log('[SERVICE]'.bold.green , '[HTTP]'.bold , 'listening'.green, 'for', '[https,spdy]'.bold, 'on' , 'https://' + address.address + ':' + address.port   );
        return started();
      });
