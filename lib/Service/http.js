...
HTTP.prototype.attach = function( maki ) {
  ...
  Object.keys( maki.resources ).forEach(function( r ) {
    ...
    Object.keys( resource.routes ).forEach(function( p ) {
      ...
      var q = {};
      var visibility = resource.options.visibility;
      
      var v = {};
      if (visibility && visibility.apply) {
        v = visibility.apply({}); // dummy object for context
      }
      ...
      if (p === 'query') {
        var executor = function(req, res, next) {
          ...
          if (visibility instanceof Array) {
            ...
            if (req.roles && req.roles.length) {
              v.$and.push({
                $or: req.roles.map(function(r) {
                  return { role: r };
                })
              });
            } else {
