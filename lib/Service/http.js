...
HTTP.prototype.attach = function( maki ) {
  ...
  maki.app.use( require('cookie-parser')( maki.config.sessions.secret ) )
  ...
  function verifyUser( username , password , done ) {
    var Person = maki.resources['Person'];
    Person.query({ username: username }, function(err, users) {
  ...
  maki.resources['Person'].pre('create', function(done) {
    var self = this;
    maki.resources['Person'].Model.register({
      email: self.email,
  ...
  maki.passport.deserializeUser(function(id, done) {
    maki.resources['Person'].query({ _id: id }, function(err, users) {
      done( err , users[0] );
    });
  });
