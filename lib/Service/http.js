...
HTTP.prototype.provide = function() {
  ...
  return function( req , res , next ) {
    ...
    var always = {};
    always[ 'config' ]    = self.maki.config;
    always[ 'resources' ] = self.maki.resources;
    always[ 'services' ]  = /**/ self.maki.services /*/ Object.keys( self.maki.services ).map(function(x) {
...
HTTP.prototype.attach = function( maki ) {
  ...
  // TODO: only `.use()` on endpoints that require it.
  // TODO: also, replace this with streams, like https://www.npmjs.com/package/stream-body-parser
  //maki.app.use( require('connect-multiparty')() );
  //var StreamBodyParser = require('stream-body-parser');
  //var bodyParser = new StreamBodyParser( maki.app );
  //maki.app.use( require('skipper')() );
  ...
  Object.keys( maki.resources ).forEach(function( r ) {
    ...
    // TODO: recurse
    Object.keys( resource.attributes ).forEach(function( f ) {
      var attribute = resource.attributes[ f ];
      
      // set up pipeline handlers for this type
      /* if (~['File'].indexOf( attribute.type )) {
        resource.pre('create', function( done ) {
          var streams = [];
          async.parallel( streams , done );
        });
      } */
      ...
      switch (p) {
        case 'query':
          ...
          if (resource.options.static) {
            return maki.app[ map[ p ] ]( regex , function(req, res, next) {
              req.m = p;
        ...
        case 'create':
          maki.app[ map[p] ]( regex ,  function(req, res, next) {
            ...
            async.map( resource._meta.files , function( filename , complete ) {
              ...
              console.log('map thing... for', filename , req.files[ filename ]);
              ...
              // this is about as fast as I could get it; no disk -> buffer
              // TODO: try to do direct GridFS streaming
              streamifier.createReadStream( file.buffer ).pipe( filestore );

            }, function(err, files) {
              // TODO: query pipeline (facades)
              var doc = req.body;
              
              console.log('grid files', files );
