      ...
      var formatters = {
        html: function() {
          ...
          var stub = {
            ...
            title: resource.options.handle ? resource.options.handle : resource.names[req.m + 'Proper'] + ' on ' + self.maki.config.service.name,
            description: (resource.options.fields.description) ? data[resource.options.fields.description] : resource.description
          };

          if (!(data instanceof Array)) {
            stub.title = (data && data.name) ? data.name.display || data.name + ', a ' + resource.names[req.m + 'Proper'] + ' on ' + self.maki.config.service.name : stub.title;
...
HTTP.prototype.attach = function( maki ) {
  ...
  function serveDocumentation(req, res, next) {
    ...
    resourceListString = safeJSON(resourceList);
    ...
    return res.format({
      ...
      json: function() {
        res.send({
          ...
          resources: JSON.parse(resourceListString)
        });
      }
    });
  }
