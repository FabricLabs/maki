...
HTTP.prototype.attach = function( maki ) {
  ...
  function filePopulator(req, res, next) {
    ...
    var ObjectID = maki.datastore.mongoose.Types.ObjectId;
        ...
        case 'create':
          maki.app[ map[p] ]( regex ,  function(req, res, next) {
            ...
            async.map( resource._meta.files , function( filename , complete ) {
              ...
              function createTorrent( innerDone ) {
                var torrent = new Torrent({ announce: 'test.com', name: 'test' });

                var torrentstore = maki.datastore.gfs.createWriteStream({
                  mode: 'w',
